...
const rewards = [
  {
    icon: Trophy,
    title: "Winner",
    desc: "Gold Medal + Cash Prize + Certification",
    color: "text-neon-gold",
  },
  {
    icon: Medal,
    title: "Runner-Up",
    desc: "Cash Prize + Certification",
    color: "text-primary",
  },
  {
    icon: Award,
    title: "Best Innovation",
    desc: "Special Award for Most Innovative Solution",
    color: "text-secondary",
  },
  {
    icon: FileCheck,
    title: "All Participants",
    desc: "Participation Certificates for Everyone",
...
const RewardsSection = () => {
  ...
  useEffect(() => {
    ...
    const increment = prizeAmount / (duration / 16);

    const counter = setInterval(() => {
      start += increment;

      if (start >= prizeAmount) {
        setCount(prizeAmount);
    ...
    <section id="prizes" className="section-padding">
      <div className="max-w-5xl mx-auto text-center">
        ...
        >
          ...
          <p className="text-sm text-muted-foreground font-heading mt-2 uppercase tracking-widest">
            Total Prize Pool
          </p>
        </motion.div>
